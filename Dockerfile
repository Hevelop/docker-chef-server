...
RUN set -x \
    && echo "\n Downloading Chef-Server" \
    && wget -q --no-check-certificate -O chef-server-core_"$CHEF_SERVER_VERSION"-1_amd64.deb "$CHEF_SERVER_DOWNLOAD_URL" \
...
RUN set -x \
    && echo "\n Downloading Chef-Manage" \
    && wget -q --no-check-certificate -O chef-manage_"$CHEF_MANAGE_VERSION"-1_amd64.deb "$CHEF_MANAGE_DOWNLOAD_URL" \
...
RUN set -x \
    && echo "\n Downloading Chef-Reporting" \
    && wget -q --no-check-certificate -O opscode-reporting_"$CHEF_REPORTING_VERSION"-1_amd64.deb "$CHEF_REPORTING_DOWNLOAD_URL" \
